...
begin_decl_stmt
DECL|variable|have_non_local_packs
specifier|static
name|int
name|have_non_local_packs
decl_stmt|;
end_decl_stmt
...
begin_function
DECL|function|want_found_object
specifier|static
name|int
name|want_found_object
parameter_list|(
name|int
name|exclude
parameter_list|,
...
literal|0
return|;
comment|/* we don't know yet; keep looking for more packs */
return|return
operator|-
literal|1
return|;
...
literal|0
return|;
comment|/* 	 * If we already know the pack object lives in, start checks from that 	 * pack - in the usual case when neither --local was given nor .keep files 	 * are present we will determine the answer right now. 	 */
if|if
condition|(
operator|*
name|found_pack
...
begin_comment
comment|/*  * Drop an on-disk delta we were planning to reuse. Naively, this would  * just involve blanking out the "delta" field, but we have to deal  * with some extra book-keeping:  *  *   1. Removing ourselves from the delta_sibling linked list.  *  *   2. Updating our size/type to the non-delta representation. These were  *      either not recorded initially (size) or overwritten with the delta type  *      (type) when check_object() decided to reuse the delta.  */
end_comment

begin_function
DECL|function|drop_reused_delta
specifier|static
name|void
name|drop_reused_delta
parameter_list|(
name|struct
name|object_entry
modifier|*
...
condition|)
block|{
comment|/* 		 * We failed to get the info from this pack for some reason; 		 * fall back to sha1_object_info, which may find another copy. 		 * And if that fails, the error will be recorded in entry->type 		 * and dealt with in prepare_pack(). 		 */
name|entry
operator|->
name|type
operator|=
...
begin_comment
comment|/*  * Follow the chain of deltas from this entry onward, throwing away any links  * that cause us to hit a cycle (as determined by the DFS state flags in  * the entries).  */
end_comment

begin_function
DECL|function|break_delta_chains
specifier|static
name|void
name|break_delta_chains
parameter_list|(
name|struct
name|object_entry
modifier|*
...
parameter_list|)
block|{
comment|/* If it's not a delta, it can't be part of a cycle. */
if|if
condition|(
operator|!
name|entry
...
name|DFS_NONE
case|:
comment|/* 		 * This is the first time we've seen the object. We mark it as 		 * part of the active potential cycle and recurse. 		 */
name|entry
operator|->
name|dfs_state
operator|=
...
name|DFS_DONE
case|:
comment|/* object already examined, and not part of a cycle */
break|break;
case|case
name|DFS_ACTIVE
case|:
comment|/* 		 * We found a cycle that needs broken. It would be correct to 		 * break any link in the chain, but it's convenient to 		 * break this one. 		 */
name|drop_reused_delta
argument_list|(
name|entry
argument_list|)
...
expr_stmt|;
block|}
comment|/* 	 * This must happen in a second pass, since we rely on the delta 	 * information for the whole list being completed. 	 */
for|for
control|(
name|i
operator|=
...
name|tag
decl_stmt|;
comment|/* 	 * We catch duplicates already in add_object_entry(), but we'd 	 * prefer to do this extra check to avoid having to parse the 	 * tag at all if we already know that it's being packed (e.g., if 	 * it was included via bitmaps, we would not have parsed it 	 * previously). 	 */
if|if
condition|(
name|packlist_find
argument_list|(
...
name|die
argument_list|(
literal|"unable to pack objects reachable from tag %s"
argument_list|,
name|oid_to_hex
argument_list|(
name|oid
...
name|p
condition|)
comment|/* no keep-able packs found */
name|ignore_packed_keep
operator|=
literal|0
expr_stmt|;
...
condition|)
block|{
comment|/* 		 * unlike ignore_packed_keep above, we do not want to 		 * unset "local" based on looking at packs, as it 		 * also covers non-local objects 		 */
name|struct
name|packed_git
modifier|*
name|p
