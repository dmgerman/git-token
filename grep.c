...
begin_function
DECL|function|compile_fixed_regexp
specifier|static
name|void
name|compile_fixed_regexp
parameter_list|(
name|struct
name|grep_pat
modifier|*
...
argument_list|)
expr_stmt|;
comment|/* 	 * Even when -F (fixed) asks us to do a non-regexp search, we 	 * may not be able to correctly case-fold when -i 	 * (ignore-case) is asked (in which case, we'll synthesize a 	 * regexp to match the pattern that matches regexp special 	 * characters literally, while ignoring case differences).  On 	 * the other hand, even without -F, if the pattern does not 	 * have any regexp special characters and there is no need for 	 * case-folding search, we can internally turn it into a 	 * simple string match using kws.  p->fixed tells us if we 	 * want to use kws. 	 */
if|if
condition|(
name|opt
operator|->
...
condition|)
block|{
comment|/* 		 * We come here when the pattern has the non-ascii 		 * characters we cannot case-fold, and asked to 		 * ignore-case. 		 */
name|compile_fixed_regexp
argument_list|(
name|p
argument_list|,
