...
name|pos
condition|)
comment|/* we have been tracking this path */
return|return
literal|1
return|;
comment|/* 	 * Look for an unmerged entry for the path, 	 * specifically stage #2, which would indicate 	 * that "our" side before the merge started 	 * had the path tracked (and resulted in a conflict). 	 */
for|for
control|(
name|pos
operator|=
...
condition|)
empty_stmt|;
comment|/* fall through, do allow dst_name to be released */
elseif|else
if|if
condition|(
name|stage
