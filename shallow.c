...
begin_comment
comment|/*  * Given rev-list arguments, run rev-list. All reachable commits  * except border ones are marked with not_shallow_flag. Border commits  * are marked with shallow_flag. The list of border/shallow commits  * are also returned.  */
end_comment

begin_function
DECL|function|get_shallow_commits_by_rev_list
name|struct
name|commit_list
modifier|*
...
name|not_shallow_flag
decl_stmt|;
comment|/* 	 * SHALLOW (excluded) and NOT_SHALLOW (included) should not be 	 * set at this point. But better be safe than sorry. 	 */
...
argument_list|)
expr_stmt|;
comment|/* Mark all reachable commits as NOT_SHALLOW */
for|for
control|(
name|p
operator|=
...
name|not_shallow_flag
expr_stmt|;
comment|/* 	 * mark border commits SHALLOW + NOT_SHALLOW. 	 * We cannot clear NOT_SHALLOW right now. Imagine border 	 * commit A is processed first, then commit B, whose parent is 	 * A, later. If NOT_SHALLOW on A is cleared at step 1, B 	 * itself is considered border at step 2, which is incorrect. 	 */
for|for
control|(
name|p
operator|=
...
argument_list|)
expr_stmt|;
comment|/* 	 * Now we can clean up NOT_SHALLOW on border commits. Having 	 * both flags set can confuse the caller. 	 */
for|for
control|(
name|p
operator|=
