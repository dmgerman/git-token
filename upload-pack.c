...
begin_comment
comment|/*  * on successful case, it's up to the caller to close cmd->out  */
end_comment

begin_function
DECL|function|do_reachable_revlist
specifier|static
name|int
name|do_reachable_revlist
parameter_list|(
name|struct
name|child_process
modifier|*
...
begin_function
DECL|function|get_reachable_list
specifier|static
name|int
name|get_reachable_list
parameter_list|(
name|struct
name|object_array
modifier|*
...
begin_function
DECL|function|has_unreachable
specifier|static
name|int
name|has_unreachable
parameter_list|(
name|struct
name|object_array
modifier|*
...
name|CLIENT_SHALLOW
expr_stmt|;
comment|/* 			 * We want to _register_ "object" as shallow, but we 			 * also need to traverse object's parents to deepen a 			 * shallow clone. Unregister it for now so we can 			 * parse and add the parents to the want list, then 			 * re-register it. 			 */
...
begin_function
DECL|function|deepen_by_rev_list
specifier|static
name|void
name|deepen_by_rev_list
parameter_list|(
name|int
name|ac
parameter_list|,
...
name|result
operator|=
name|get_shallow_commits_by_rev_list
argument_list|(
name|ac
argument_list|,
name|av
...
name|deepen_since
operator|||
comment|/* revisions.c's max_age -1 is special */
name|deepen_since
operator|==
operator|-
literal|1
...
name|die
argument_list|(
literal|"Invalid deepen-since: %s"
argument_list|,
name|line
argument_list|)
expr_stmt|;
...
name|die
argument_list|(
literal|"git upload-pack: ambiguous deepen-not: %s"
argument_list|,
name|line
argument_list|)
expr_stmt|;
...
name|die
argument_list|(
literal|"git upload-pack: deepen and deepen-since (or deepen-not) cannot be used together"
argument_list|)
expr_stmt|;
if|if
condition|(
...
literal|" side-band-64k ofs-delta shallow deepen-since deepen-not"
literal|" deepen-relative no-progress include-tag multi_ack_detailed"
decl_stmt|;
specifier|const
name|char
modifier|*
