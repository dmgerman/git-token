...
name|die
argument_list|(
literal|"BUG: worktree status add???"
argument_list|)
expr_stmt|;
break|break;
case|case
...
argument_list|)
expr_stmt|;
comment|/* mode_worktree is zero for a delete. */
break|break;
case|case
name|DIFF_STATUS_MODIFIED
case|:
...
name|die
argument_list|(
literal|"BUG: worktree status unknown???"
argument_list|)
expr_stmt|;
break|break;
block|}
...
name|DIFF_STATUS_ADDED
case|:
comment|/* Leave {mode,oid}_head zero for an add. */
name|d
operator|->
name|mode_index
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Leave {mode,oid}_index zero for a delete. */
break|break;
case|case
name|DIFF_STATUS_COPIED
case|:
...
argument_list|)
expr_stmt|;
comment|/* 			 * Don't bother setting {mode,oid}_{head,index} since the print 			 * code will output the stage values directly and not use the 			 * values in these fields. 			 */
break|break;
block|}
block|}
block|}
...
name|DIFF_STATUS_ADDED
expr_stmt|;
comment|/* Leave {mode,oid}_head zero for adds. */
name|d
operator|->
name|mode_index
operator|=
...
begin_comment
comment|/*  * Print branch information for porcelain v2 output.  These lines  * are printed when the '--branch' parameter is given.  *  *    # branch.oid<commit><eol>  *    # branch.head<head><eol>  *   [# branch.upstream<upstream><eol>  *   [# branch.ab +<ahead> -<behind><eol>]]  *  *<commit> ::= the current commit hash or the the literal  *                   "(initial)" to indicate an initialized repo  *                   with no commits.  *  *<head> ::=<branch_name> the current branch name or  *                   "(detached)" literal when detached head or  *                   "(unknown)" when something is wrong.  *  *<upstream> ::= the upstream branch name, when set.  *  *<ahead> ::= integer ahead value, when upstream set  *                   and the commit is present (not gone).  *  *<behind> ::= integer behind value, when upstream set  *                   and commit is present.  *  *  * The end-of-line is defined by the -z flag.  *  *<eol> ::= NUL when -z,  *                           LF when NOT -z.  *  */
end_comment

begin_function
DECL|function|wt_porcelain_v2_print_tracking
specifier|static
name|void
name|wt_porcelain_v2_print_tracking
parameter_list|(
name|struct
name|wt_status
modifier|*
...
expr_stmt|;
block|}
comment|/* Lookup stats on the upstream tracking branch, if set. */
name|branch
operator|=
name|branch_get
argument_list|(
...
begin_comment
comment|/*  * Convert various submodule status values into a  * fixed-length string of characters in the buffer provided.  */
end_comment

begin_function
DECL|function|wt_porcelain_v2_submodule_state
specifier|static
name|void
name|wt_porcelain_v2_submodule_state
parameter_list|(
name|struct
name|wt_status_change_data
modifier|*
...
begin_comment
comment|/*  * Fix-up changed entries before we print them.  */
end_comment

begin_function
DECL|function|wt_porcelain_v2_fix_up_changed
specifier|static
name|void
name|wt_porcelain_v2_fix_up_changed
parameter_list|(
name|struct
name|string_list_item
modifier|*
...
condition|)
block|{
comment|/* 		 * This entry is unchanged in the index (relative to the head). 		 * Therefore, the collect_updated_cb was never called for this 		 * entry (during the head-vs-index scan) and so the head column 		 * fields were never set. 		 * 		 * We must have data for the index column (from the 		 * index-vs-worktree scan (otherwise, this entry should not be 		 * in the list of changes)). 		 * 		 * Copy index column fields to the head column, so that our 		 * output looks complete. 		 */
name|assert
argument_list|(
name|d
operator|->
...
condition|)
block|{
comment|/* 		 * This entry is unchanged in the worktree (relative to the index). 		 * Therefore, the collect_changed_cb was never called for this entry 		 * (during the index-vs-worktree scan) and so the worktree column 		 * fields were never set. 		 * 		 * We must have data for the index column (from the head-vs-index 		 * scan). 		 * 		 * Copy the index column fields to the worktree column so that 		 * our output looks complete. 		 * 		 * Note that we only have a mode field in the worktree column 		 * because the scan code tries really hard to not have to compute it. 		 */
name|assert
argument_list|(
name|d
operator|->
...
begin_comment
comment|/*  * Print porcelain v2 info for tracked entries with changes.  */
end_comment

begin_function
DECL|function|wt_porcelain_v2_print_changed_entry
specifier|static
name|void
name|wt_porcelain_v2_print_changed_entry
parameter_list|(
name|struct
name|string_list_item
modifier|*
...
condition|)
block|{
comment|/* 		 * In -z mode, we DO NOT C-quote pathnames.  Current path is ALWAYS first. 		 * A single NUL character separates them. 		 */
name|sep_char
operator|=
literal|'\0'
expr_stmt|;
...
block|{
comment|/* 		 * Path(s) are C-quoted if necessary. Current path is ALWAYS first. 		 * The source path is only present when necessary. 		 * A single TAB separates them (because paths can contain spaces 		 * which are not escaped and C-quoting does escape TAB characters). 		 */
name|sep_char
operator|=
literal|'\t'
expr_stmt|;
...
name|fp
argument_list|,
literal|"2 %s %s %06o %06o %06o %s %s %c%d %s%c%s%c"
argument_list|,
name|key
argument_list|,
name|submodule_token
...
name|fp
argument_list|,
literal|"1 %s %s %06o %06o %06o %s %s %s%c"
argument_list|,
name|key
argument_list|,
name|submodule_token
...
begin_comment
comment|/*  * Print porcelain v2 status info for unmerged entries.  */
end_comment

begin_function
DECL|function|wt_porcelain_v2_print_unmerged_entry
specifier|static
name|void
name|wt_porcelain_v2_print_unmerged_entry
parameter_list|(
name|struct
name|string_list_item
modifier|*
...
expr_stmt|;
block|}
comment|/* 	 * Disregard d.aux.porcelain_v2 data that we accumulated 	 * for the head and index columns during the scans and 	 * replace with the actual stage data. 	 * 	 * Note that this is a last-one-wins for each the individual 	 * stage [123] columns in the event of multiple cache entries 	 * for same stage. 	 */
name|memset
argument_list|(
name|stages
argument_list|,
...
name|die
argument_list|(
literal|"BUG: observed stagemask 0x%x != expected stagemask 0x%x"
argument_list|,
name|sum
argument_list|,
name|d
...
name|fp
argument_list|,
literal|"%c %s %s %06o %06o %06o %06o %s %s %s %s%c"
argument_list|,
name|unmerged_prefix
argument_list|,
name|key
...
begin_comment
comment|/*  * Print porcelain V2 status info for untracked and ignored entries.  */
end_comment

begin_function
DECL|function|wt_porcelain_v2_print_other
specifier|static
name|void
name|wt_porcelain_v2_print_other
parameter_list|(
name|struct
name|string_list_item
modifier|*
...
begin_comment
comment|/*  * Print porcelain V2 status.  *  * [<v2_branch>]  * [<v2_changed_items>]*  * [<v2_unmerged_items>]*  * [<v2_untracked_items>]*  * [<v2_ignored_items>]*  *  */
end_comment

begin_function
DECL|function|wt_porcelain_v2_print
specifier|static
name|void
name|wt_porcelain_v2_print
parameter_list|(
name|struct
name|wt_status
modifier|*
...
break|break;
case|case
name|STATUS_FORMAT_PORCELAIN_V2
case|:
name|wt_porcelain_v2_print
argument_list|(
name|s
